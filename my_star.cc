...
main (int argc, char *argv[])
{
  ...
  UdpClientHelper udpClient (hubLocalAddress);
  ...
  for (uint32_t i = 0; i < star.SpokeCount (); ++i)
    {
      ...
      udpClient.SetAttribute ("RemoteAddress", remoteAddress);
      spokeApps.Add (udpClient.Install (star.GetSpokeNode (i)));
    }
  spokeApps.Start (Seconds (1.0));
  // spokeApps.Stop (Seconds (10.0));
  spokeApps.Stop (Seconds (5.0));
