...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer sender1 = p2p1.Install (senders.Get(0), senders.Get(1));
  NetDeviceContainer sender2 = p2p1.Install (senders.Get(2), senders.Get(1));
  NetDeviceContainer sender3 = p2p1.Install (senders.Get(3), senders.Get(1));
  NetDeviceContainer reciever = p2p2.Install (recievers);
  ...
  ApplicationContainer apps = sink.Install (recievers.Get (1));
  ...
  ApplicationContainer sourceApps1 = udpClient.Install (senders.Get (0));
  sourceApps1.Start (Seconds (1.0));
  sourceApps1.Stop (Seconds(2.0));
  ApplicationContainer sourceApps2 = udpClient.Install (senders.Get (2));
  sourceApps2.Start (Seconds (1.0));
  sourceApps2.Stop (Seconds(2.0));
  ApplicationContainer sourceApps3 = udpClient.Install (senders.Get (3));
  sourceApps2.Start (Seconds (1.0));
  sourceApps2.Stop (Seconds(2.0));
